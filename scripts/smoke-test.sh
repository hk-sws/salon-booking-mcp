...
# --- reads -----------------------------------------------------------------
run GET "$B/business?companyId=$CO"
if [ "$CODE" = "200" ] && echo "$BODY" | grep -q "Closed on Mondays"; then
  ok "GET /business (hoursSpoken collapsed)"; else no "GET /business" show; fi

run GET "$B/business?companyId=ghost"
expect 404 "GET /business bad tenant -> 404"

run GET "$B/services?companyId=$CO"
if [ "$CODE" = "200" ] && echo "$BODY" | grep -q "svc_haircut"; then
  ok "GET /services"; else no "GET /services" show; fi

run GET "$B/services?companyId=$CO&bookableOnly=true"
expect 200 "GET /services?bookableOnly=true"

run GET "$B/stylists?companyId=$CO"
expect 200 "GET /stylists"

run GET "$B/stylists?companyId=$CO&serviceId=svc_highlights"
...
run POST "$B/resolve-date" '{"companyId":"salon-01","phrase":"tomorrow"}'
[ "$(pyget "['confident']")" = "True" ] && ok "POST /resolve-date confident" || no "resolve-date" show

run POST "$B/resolve-date" '{"companyId":"salon-01","phrase":"sometime soon"}'
...
run GET "$B/availability?companyId=$CO&serviceId=svc_haircut&dateFrom=$DFROM&dateTo=$DTO&limit=6"
AV="$BODY"
SCOUNT="$(pyget "['count']")"
...
run GET "$B/availability?companyId=$CO&serviceId=svc_haircut&date=$NEXTMON"
...
run GET "$B/availability?companyId=$CO&serviceId=svc_bridal&date=$S0_DATE"
expect 403 "GET /availability non-bookable -> 403"

run GET "$B/availability?companyId=$CO&serviceId=svc_highlights&date=$S0_DATE&stylistId=sty_ramesh"
[ "$(pyget "['error']")" = "STYLIST_SERVICE_MISMATCH" ] && ok "GET /availability mismatch -> 400" || no "availability mismatch" show

run GET "$B/availability?companyId=$CO&date=$S0_DATE"
expect 400 "GET /availability missing serviceId -> 400"

# --- bookings lifecycle ----------------------------------------------------
echo
echo "Bookings"
BODYJSON="{\"companyId\":\"$CO\",\"serviceId\":\"svc_haircut\",\"stylistId\":\"$S0_STY\",\"date\":\"$S0_DATE\",\"start\":\"$S0_START\",\"customer\":{\"name\":\"Smoke\",\"phone\":\"9000000777\",\"email\":\"smoke@example.com\"}}"
...
run GET "$B/bookings/find?companyId=$CO&phone=9000000777"
...
run POST "$B/bookings/$BID/cancel" "{\"companyId\":\"$CO\",\"reason\":\"smoke test\"}"
...
run GET "$B/availability?companyId=$CO&serviceId=svc_haircut&date=$TOM&limit=6"
NDATE="$(pyget "['slots'][0]['date']")"; NSTART="$(pyget "['slots'][0]['start']")"; NSTY="$(pyget "['slots'][0]['stylistId']")"
if [ -n "$NDATE" ]; then
  run POST "$B/bookings" "{\"companyId\":\"$CO\",\"serviceId\":\"svc_haircut\",\"stylistId\":\"$NSTY\",\"date\":\"$NDATE\",\"start\":\"$NSTART\",\"customer\":{\"name\":\"Near\",\"phone\":\"9000000888\"}}"
  NBID="$(pyget "['bookingId']")"
  if [ -n "$NBID" ]; then
    run POST "$B/bookings/$NBID/cancel" "{\"companyId\":\"$CO\",\"reason\":\"late\"}"
...
run POST "$B/messages" "{\"companyId\":\"$CO\",\"category\":\"bridal\",\"name\":\"Meera\",\"phone\":\"9000000001\",\"note\":\"December wedding\"}"
expect 201 "POST /messages -> 201"

run GET "$B/admin/bookings?companyId=$CO&status=confirmed"
expect 200 "GET /admin/bookings"

run GET "$B/admin/messages?companyId=$CO&status=open"
expect 200 "GET /admin/messages"

run POST "$B/admin/blocks" "{\"companyId\":\"$CO\",\"stylistId\":\"sty_priya\",\"startAt\":\"${S1_DATE}T04:30:00.000Z\",\"endAt\":\"${S1_DATE}T14:30:00.000Z\",\"reason\":\"smoke block\"}"
expect 201 "POST /admin/blocks -> 201"

run POST "$B/admin/seed?companyId=$CO"
